...
function decodeUplink(input) {
    ...
    if(input.fPort==126){
        ...
        if(raw.byteLength == 10 && (intput_list[0]==0x00)&&(intput_list[3]==0x03)&&(intput_list[6]==0x04)&&(intput_list[8]==0x05)){
            ...
            result.data.cfg_detect = parseInt(detectt_hex,16).toString(16); //Set detection interval (sec)
            result.data.cfg_tamper = parseInt(intput_list[7],16).toString(16); //Enable tamper detection
            result.data.cfg_buzzer = parseInt(intput_list[9],16).toString(16); //Buzzer alarm period times(sec)
            return result;
        }
