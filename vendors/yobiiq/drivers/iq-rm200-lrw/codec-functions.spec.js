...
function runFunction(name, input)
{
    switch (name) {
        case 'decodeBasicInformation':
            let ret = codec.decodeBasicInformation(input)
            versionKeys.forEach(key => {
                delete ret[key];
            });
            return ret;
        ...
        case 'decodeAlarmPacket':
            return codec.decodeAlarmPacket(input)
        case 'decodeParameters':
            return codec.decodeParameters(input)
        case 'encodeDeviceConfiguration':
            return codec.encodeDeviceConfiguration(input)
        case 'encodeUplinkConfiguration':
            return codec.encodeUplinkConfiguration(input)
        case 'encodeParametersReading':
            return codec.encodeParametersReading(input)
        default:
            break;
    }
