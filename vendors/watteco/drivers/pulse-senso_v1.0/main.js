...
// ../../device-catalog/vendors/watteco/drivers/pulse-senso_v1.0/extractPoints.js
var require_extractPoints = __commonJS({
  "../../device-catalog/vendors/watteco/drivers/pulse-senso_v1.0/extractPoints.js"(exports2) {
    function extractPoints(input) {
        ...
        data.forEach((item) => {
          if (item.variable != null) {
            switch (item.variable) {
              ...
              case "pin_state_3":
                if (!result["status:3"]) {
                  result["status:3"] = { unitId: "state", records: [] };
                }
                result["status:3"].records.push({
                  eventTime: new Date(item.date).toISOString(),
                  value: item.value
                });
                break;
              ...
              case "index_3":
                if (!result["frequency:3"]) {
                  result["frequency:3"] = { unitId: "pulse/h", records: [] };
                }
      ...
      if (result["status:3"] != null && result["status:3"].records.length === 1) {
        let val = result["status:3"].records[0].value;
        result["status:3"] = { unitId: "state", record: val };
      }
      ...
      if (result["frequency:3"] != null && result["frequency:3"].records.length === 1) {
        ...
        result["frequency:3"] = { unitId: "pulse/h", record: val };
      }
      return result;
    }
...
// ../../device-catalog/vendors/watteco/drivers/pulse-senso_v1.0/main.js
var watteco = require_decode();
var batch_param = [4, [
  { taglbl: 0, resol: 1, sampletype: 10, lblname: "index_1", divide: 1 },
  ...
  { taglbl: 6, resol: 100, sampletype: 6, lblname: "battery_voltage", divide: 1e3 },
