...
// ../../device-catalog/vendors/watteco/drivers/vaqao-plus_v1.0/extractPoints.js
var require_extractPoints = __commonJS({
  "../../device-catalog/vendors/watteco/drivers/vaqao-plus_v1.0/extractPoints.js"(exports2) {
    function extractPoints(input) {
            ...
            switch (item.variable) {
              ...
              case "temperature_2":
                if (!result["temperature:2"]) {
                  result["temperature:2"] = { unitId: "Cel", records: [] };
                }
                result["temperature:2"].records.push({
                  eventTime: new Date(item.date).toISOString(),
                  value: item.value
                });
                break;
              case "humidity_2":
                if (!result["humidity:2"]) {
                  result["humidity:2"] = { unitId: "%RH", records: [] };
                }
                result["humidity:2"].records.push({
                  eventTime: new Date(item.date).toISOString(),
                  value: item.value
                });
                break;
      ...
      if (result["humidity:2"] != null && result["humidity:2"].records.length === 1) {
        let val = result["humidity:2"].records[0].value;
        result["humidity:2"] = { unitId: "%RH", record: val };
      }
      ...
      if (result["temperature:2"] != null && result["temperature:2"].records.length === 1) {
        let val = result["temperature:2"].records[0].value;
        result["temperature:2"] = { unitId: "Cel", record: val };
      }
      return result;
    }
...
// ../../device-catalog/vendors/watteco/drivers/vaqao-plus_v1.0/main.js
var watteco = require_decode();
var batch_param = [3, [
  { taglbl: 0, resol: 1, sampletype: 4, lblname: "occupancy", divide: 1 },
