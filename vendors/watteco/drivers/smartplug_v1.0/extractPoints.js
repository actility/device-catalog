function extractPoints(input) {
                ...
                switch (item.variable) {
                    ...
                    case "frequency":
                        if (!result["frequency:1"]) {
                            result["frequency:1"] = { unitId: "hertz", records: [], nature: "current frequency" }
                        }
                    ...
                    case "frequency_min":
                        if (!result["frequency:2"]) {
                            result["frequency:2"] = { unitId: "hertz", records: [], nature: "frequency min" }
                        }
                    ...
                    case "frequency_max":
                        if (!result["frequency:3"]) {
                            result["frequency:3"] = { unitId: "hertz", records: [], nature: "frequency max" }
                        }
                        result["frequency:3"].records.push({
                            eventTime: new Date(item.date).toISOString(),
                            value: item.value/1000
                        })
                        break
                    ...
                    case "Vrms":
                        if (!result["rmsVoltage:1"]) {
                            result["rmsVoltage:1"] = { unitId: "V", records: [], nature: "Current Vrms" }
                        }
                    ...
                    case "Vrms_min":
                        if (!result["rmsVoltage:2"]) {
                            result["rmsVoltage:2"] = { unitId: "V", records: [], nature: "Vrms min" }
                        }
                    ...
                    case "Vrms_max":
                        if (!result["rmsVoltage:3"]) {
                            result["rmsVoltage:3"] = { unitId: "V", records: [], nature: "Vrms max" }
                        }
                    ...
                    case "Vpeak":
                        if (!result["voltage:1"]) {
                            result["voltage:1"] = { unitId: "V", records: [], nature: "Current volt peak" }
                        }
                    ...
                    case "Vpeak_min":
                        if (!result["voltage:2"]) {
                            result["voltage:2"] = { unitId: "V", records: [], nature: "volt min" }
                        }
                    ...
                    case "Vpeak_max":
                        if (!result["voltage:3"]) {
                            result["voltage:3"] = { unitId: "V", records: [], nature: "volt max" }
                        }
                        result["voltage:3"].records.push({
                            eventTime: new Date(item.date).toISOString(),
                            value: item.value/10
                        })
                        break
                    ...
                    case "over_voltage":
                        if (!result["counter:1"]) {
                            result["counter:1"] = { unitId: "count", records: [], nature:"Over voltage Threshold count"}
                        }
                    ...
                    case "sag_voltage":
                        if (!result["counter:2"]) {
                            result["counter:2"] = { unitId: "count", records: [], nature:"Sag voltage Threshold count"}
                        }
                        result["counter:2"].records.push({
                            eventTime: new Date(item.date).toISOString(),
                            value: item.value/10
                        })
                        break
    ...
    if(result["frequency:1"] != null && result["frequency:1"].records.length === 1) {
        ...
        result["frequency:1"] = { unitId: "hertz", record: val, nature: "current frequency" }
    }

    if(result["frequency:2"] != null && result["frequency:2"].records.length === 1) {
        let val = result["frequency:2"].records[0].value
        result["frequency:2"] = { unitId: "hertz", record: val, nature: "frequency min" }
    }

    if(result["frequency:3"] != null && result["frequency:3"].records.length === 1) {
        let val = result["frequency:3"].records[0].value
        result["frequency:3"] = { unitId: "hertz", record: val, nature: "frequency max" }
    }

    if(result["rmsVoltage:1"] != null && result["rmsVoltage:1"].records.length === 1) {
        let val = result["rmsVoltage:1"].records[0].value
        result["rmsVoltage:1"] = { unitId: "V", record: val, nature: "current Vrms" }
    }

    if(result["rmsVoltage:2"] != null && result["rmsVoltage:2"].records.length === 1) {
        let val = result["rmsVoltage:2"].records[0].value
        result["rmsVoltage:2"] = { unitId: "V", record: val, nature: "Vrms min" }
    }

    if(result["rmsVoltage:3"] != null && result["rmsVoltage:3"].records.length === 1) {
        let val = result["rmsVoltage:3"].records[0].value
        result["rmsVoltage:3"] = { unitId: "V", record: val, nature: "Vrms max" }
    }

    if(result["counter:1"] != null && result["counter:1"].records.length === 1) {
        let val = result["counter:1"].records[0].value
        result["counter:1"] = { unitId: "count", record: val, nature:"Over voltage Threshold count" }
    }

    if(result["counter:2"] != null && result["counter:2"].records.length === 1) {
        let val = result["counter:2"].records[0].value
        result["counter:2"] = { unitId: "count", record: val, nature:"Sag voltage Threshold count" }
    }

    if(result["voltage:1"] != null && result["voltage:1"].records.length === 1) {
        let val = result["voltage:1"].records[0].value
        result["voltage:1"] = { unitId: "V", record: val, nature:"current volt peak" }
    }

    if(result["voltage:2"] != null && result["voltage:2"].records.length === 1) {
        let val = result["voltage:2"].records[0].value
        result["voltage:2"] = { unitId: "V", record: val, nature:"volt min" }
    }

    if(result["voltage:3"] != null && result["voltage:3"].records.length === 1) {
        let val = result["voltage:3"].records[0].value
        result["voltage:3"] = { unitId: "V", record: val, nature:"volt max" }
    }
