...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // GPIO 1 (INPUT)
            ...
            decoded.gpio_input_1 = readGPIOStatus(bytes[i]);
            i += 1;
        }
        // GPIO 2 (INPUT)
        else if (channel_id === 0x02 && channel_type !== 0xc8) {
            decoded.gpio_input_2 = readGPIOStatus(bytes[i]);
            i += 1;
        }
        // GPIO 1 (COUNTER)
        else if (channel_id === 0x01 && channel_type === 0xc8) {
        ...
        // GPIO 1 (OUTPUT)
            ...
            decoded.gpio_output_1 = readGPIOStatus(bytes[i]);
            i += 1;
        }
        // GPIO 2 (OUTPUT)
        else if (channel_id === 0x0a) {
            decoded.gpio_output_2 = readGPIOStatus(bytes[i]);
            i += 1;
        }
        // ADC 1
