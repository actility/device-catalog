...
function milesightDeviceEncode(payload) {
    ...
    if ("humidity_chn1_calibration_settings" in payload) {
        encoded = encoded.concat(setHumidityCalibration(2, payload.humidity_chn1_calibration_settings));
    }
    if ("humidity_chn2_calibration_settings" in payload) {
        encoded = encoded.concat(setHumidityCalibration(3, payload.humidity_chn2_calibration_settings));
    }
    ...
    if ("temperature_chn1_alarm_config" in payload) {
        encoded = encoded.concat(setTemperatureAlarmConfig(0, payload.temperature_chn1_alarm_config));
    }
    if ("temperature_chn2_alarm_config" in payload) {
        encoded = encoded.concat(setTemperatureAlarmConfig(1, payload.temperature_chn2_alarm_config));
    }
    if ("temperature_chn1_mutation_alarm_config" in payload) {
        encoded = encoded.concat(setTemperatureMutationAlarmConfig(2, payload.temperature_chn1_mutation_alarm_config));
    }
    if ("temperature_chn2_mutation_alarm_config" in payload) {
        encoded = encoded.concat(setTemperatureMutationAlarmConfig(3, payload.temperature_chn2_mutation_alarm_config));
    }
    if ("humidity_chn1_alarm_config" in payload) {
        encoded = encoded.concat(setHumidityAlarmConfig(4, payload.humidity_chn1_alarm_config));
    }
    if ("humidity_chn2_alarm_config" in payload) {
        encoded = encoded.concat(setHumidityAlarmConfig(5, payload.humidity_chn2_alarm_config));
    }
    if ("humidity_chn1_mutation_alarm_config" in payload) {
        encoded = encoded.concat(setHumidityMutationAlarmConfig(6, payload.humidity_chn1_mutation_alarm_config));
    }
    if ("humidity_chn2_mutation_alarm_config" in payload) {
        encoded = encoded.concat(setHumidityMutationAlarmConfig(7, payload.humidity_chn2_mutation_alarm_config));
    }
 ...
 * @example { temperature_chn1_calibration_settings": { "enable": 1, "calibration_value": 25 } }
 */
function setTemperatureCalibration(idx, temperature_calibration_settings) {
    ...
    var idx_chn_map = { 0: "chn1", 1: "chn2" };
    var chn_name = getChannelName(idx_chn_map, idx);
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("temperature_" + chn_name + "_calibration_settings.enable must be one of " + enable_values.join(", "));
    }
 ...
 * @example { humidity_chn1_calibration_settings": { "enable": 1, "calibration_value": 5 } }
 */
function setHumidityCalibration(idx, humidity_calibration_settings) {
    ...
    var idx_chn_map = { 2: "chn1", 3: "chn2" };
    var chn_name = getChannelName(idx_chn_map, idx);
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("humidity_" + chn_name + "_calibration_settings.enable must be one of " + enable_values.join(", "));
    }
 ...
 * @example { "temperature_chn1_alarm_config": { "enable": 1, "condition": 1, "threshold_min": 20, "threshold_max": 30 } }
 */
function setTemperatureAlarmConfig(idx, temperature_alarm_config) {
    ...
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("temperature_" + chn_name + "_alarm_config.enable must be one of " + enable_values.join(", "));
    }
    var condition_map = { 0: "disable", 1: "below", 2: "above", 3: "between", 4: "outside" };
    var condition_values = getValues(condition_map);
    if (condition_values.indexOf(condition) === -1) {
        throw new Error("temperature_" + chn_name + "_alarm_config.condition must be one of " + condition_values.join(", "));
    }
 ...
 * @example { "temperature_chn1_mutation_alarm_config": { "enable": 1, "mutation": 1, "alarm_release_enable": 1 } }
 */
function setTemperatureMutationAlarmConfig(idx, temperature_mutation_alarm_config) {
    ...
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("temperature_" + chn_name + "_mutation_alarm_config.enable must be one of " + enable_values.join(", "));
    }
 ...
 * @example { "humidity_chn1_alarm_config": { "enable": 1, "condition": 1, "threshold_min": 30, "threshold_max": 70 } }
 */
function setHumidityAlarmConfig(idx, humidity_alarm_config) {
    ...
    var idx_chn_map = { 4: "chn1", 5: "chn2" };
    var chn_name = getChannelName(idx_chn_map, idx);
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("humidity_" + chn_name + "_alarm_config.enable must be one of " + enable_values.join(", "));
    }

    var condition_map = { 0: "disable", 1: "below", 2: "above", 3: "between", 4: "outside" };
    var condition_values = getValues(condition_map);
    if (condition_values.indexOf(condition) === -1) {
        throw new Error("humidity_" + chn_name + "_alarm_config.condition must be one of " + condition_values.join(", "));
    }

    var data = 0x00;
    data |= getValue(condition_map, condition);
    data |= idx << 3; // humidity alarm
    data |= getValue(enable_map, enable) << 6;
 ...
 * @example { "humidity_chn1_mutation_alarm_config": { "enable": 1, "mutation": 1 } }
 */
function setHumidityMutationAlarmConfig(idx, humidity_mutation_alarm_config) {
    var enable = humidity_mutation_alarm_config.enable;
    var mutation = humidity_mutation_alarm_config.mutation;

    var idx_chn_map = { 6: "chn1", 7: "chn2" };
    var chn_name = getChannelName(idx_chn_map, idx);
    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("humidity_" + chn_name + "_mutation_alarm_config.enable must be one of " + enable_values.join(", "));
    }

    var data = 0x00;
    data |= 5; // mutation
    data |= idx << 3; // humidity mutation alarm
    data |= getValue(enable_map, enable) << 6;
...
/**
 * fetch sensor id
 * @param {number} fetch_sensor_id values: (0: sensor_1, 1: sensor_2, 2: all)
 * @example { "fetch_sensor_id": 0 }
 */
function fetchSensorID(fetch_sensor_id) {
    var sensor_map = { 0: "sensor_1", 1: "sensor_2", 2: "all" };
 ...
 * @param {number} d2d_master_config._item.mode values: (1: temperature_chn1_threshold_alarm, 2: temperature_chn1_threshold_alarm_release, 3: temperature_chn1_mutation_alarm, 4: humidity_chn1_threshold_alarm, 5: humidity_chn1_threshold_alarm_release, 6: humidity_chn1_mutation_alarm, 7: magnet_chn1_close, 8: magnet_chn1_open, 9: temperature_chn2_threshold_alarm, 10: temperature_chn2_threshold_alarm_release, 11: temperature_chn2_mutation_alarm, 12: humidity_chn2_threshold_alarm, 13: humidity_chn2_threshold_alarm_release, 14: humidity_chn2_mutation_alarm, 15: magnet_chn2_close, 16: magnet_chn2_open)
 ...
 * @param {number} d2d_uplink_config.sensor_data_config.temperature_chn1 values: (0: disable, 1: enable)
 * @param {number} d2d_uplink_config.sensor_data_config.humidity_chn1 values: (0: disable, 1: enable)
 * @param {number} d2d_uplink_config.sensor_data_config.temperature_chn2 values: (0: disable, 1: enable)
 * @param {number} d2d_uplink_config.sensor_data_config.humidity_chn2 values: (0: disable, 1: enable)
 * @example { "d2d_uplink_config": { "d2d_uplink_enable": 1, "lora_uplink_enable": 1, "sensor_data_config": { "temperature_chn1": 1, "humidity_chn1": 1, "temperature_chn2": 1, "humidity_chn2": 1 } } }
 */
 ...
 * @param {number} query_config.temperature_chn1_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.humidity_chn1_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.temperature_chn2_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.humidity_chn2_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.temperature_chn1_mutation_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.humidity_chn1_mutation_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.temperature_chn2_mutation_alarm_config values: (0: disable, 1: enable)
 * @param {number} query_config.humidity_chn2_mutation_alarm_config values: (0: disable, 1: enable)
 ...
 * @param {number} query_config.d2d_master_config_with_temperature_chn1_threshold_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_temperature_chn1_threshold_alarm_release values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_temperature_chn1_mutation_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn1_threshold_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn1_threshold_alarm_release values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn1_mutation_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_magnet_chn1_close_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_magnet_chn1_open_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_temperature_chn2_threshold_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_temperature_chn2_threshold_alarm_release values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_temperature_chn2_mutation_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn2_threshold_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn2_threshold_alarm_release values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_humidity_chn2_mutation_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_magnet_chn2_close_alarm values: (0: disable, 1: enable)
 * @param {number} query_config.d2d_master_config_with_magnet_chn2_open_alarm values: (0: disable, 1: enable)
...
function getChannelName(map, idx) {
    return map[idx];
}
