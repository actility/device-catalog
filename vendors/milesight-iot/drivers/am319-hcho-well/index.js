...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // HISTORY DATA (AM319 CH2O)
            ...
            data.hcho = readUInt16LE(bytes.slice(i + 18, i + 20)) / 1000;
            i += 20;
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x6d:
            ...
            break;
        case 0xac:
            decoded.pm_self_clean_config = {};
            decoded.pm_self_clean_config.enable = readEnableStatus(bytes[offset]);
            decoded.pm_self_clean_config.period = readUInt8(bytes[offset + 1]);
            offset += 2;
            break;
        case 0xad:
            decoded.pm_self_clean = readYesNoStatus(1);
            offset += 1;
            break;
        case 0xf0:
...
function milesightDeviceEncode(payload) {
    ...
    if ("pm_self_clean_config" in payload) {
        encoded = encoded.concat(setPMSelfCleanConfig(payload.pm_self_clean_config));
    }
    if ("pm_self_clean" in payload) {
        encoded = encoded.concat(PMSelfCleanConfig(payload.pm_self_clean));
    }
...
/**
 * Set PM sensor self clean config
 * @param {object} pm_self_clean_config
 * @param {number} pm_self_clean_config.enable values: (0: disable, 1: enable)
 * @param {number} pm_self_clean_config.period unit: day, range: [1, 255]
 * @example { "pm_self_clean_config": { "enable": 1, "period": 1 } }
 */
function setPMSelfCleanConfig(pm_self_clean_config) {
    var enable = pm_self_clean_config.enable;
    var period = pm_self_clean_config.period;

    var enable_map = { 0: "disable", 1: "enable" };
    var enable_values = getValues(enable_map);
    if (enable_values.indexOf(enable) === -1) {
        throw new Error("pm_self_clean_config.enable must be one of " + enable_values.join(", "));
    }
    if (period < 1 || period > 255) {
        throw new Error("pm_self_clean_config.period must be between 1 and 255");
    }
...
/**
 * PM sensor self clean
 * @param {number} pm_self_clean values: (0: no, 1: yes)
 * @example { "pm_self_clean": 1 }
 */
function PMSelfCleanConfig(pm_self_clean) {
    var yes_no_map = { 0: "no", 1: "yes" };
    var yes_no_values = getValues(yes_no_map);
    if (yes_no_values.indexOf(pm_self_clean) === -1) {
        throw new Error("pm_self_clean must be one of " + yes_no_values.join(", "));
    }

    if (getValue(yes_no_map, pm_self_clean) === 0) {
        return [];
    }
    return [0xf9, 0xad, 0xff];
}
