...
function decode_sensor_event_msg_normal(bytes, cursor, velocity_conversion, temperature_conversion) {
  ...
  // byte[3,4]
  const x = convert_unit(velocity_conversion, decode_uint16(bytes, cursor) / 100);

  // byte[5,6]
  const y = convert_unit(velocity_conversion, decode_uint16(bytes, cursor) / 100);

  // byte[7,8]
  const z = convert_unit(velocity_conversion, decode_uint16(bytes, cursor) / 100);

  // byte[9,10]
  const temperature = convert_unit(temperature_conversion, decode_int16(bytes, cursor) / 100);
