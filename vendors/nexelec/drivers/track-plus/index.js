/* 
 * Payload Decoder LoRa Alliance for TRACK+ (X950)
...
function decodeUplink(input) 
{
    ...
    {
        var message_name =["Correct orientation","Incorrect orientation","Information not available"]
        return message_name[octetOrientation]
    }

    function workingMode(octetWorking)
    {
        var message_name =["Normal","Low battery","Preservation mode"]
        return message_name[octetWorking]
    }

    function powerSource(octetPowerSource)
    {
        var message_name =["External(AC,DC or PV)","Internal battery"]
    ...
    {
        var message_name =["NFC","Downlink","Product Start up","Network","GPS","Local"]
        return message_name[octetReconfigurationSource]
    }

    function reconfigurationState(octetReconfigurationState)
    {
        var message_name =["Total Succes","Partial Succes","Total fail","Reserved"]
    ...
    {
        var message_name =["Reserved","EU868","Reserved","Reserved","Reserved","Reserved","Reserved","Reserved","Reserved"]
    ...
    {
        var message_name =["Periodic","Continuous"]
        ...
        while(i < stringHex.length){
            ...
            switch (channel){
                ...
                case CHANNEL_PM_CHANNEL_4_centi :
                    let data_pm0_5_c_div_100 = (parseInt(stringHex.substring(i+2,i+6),16)) & 0xFFFF;
                    data.pm0_5_c_div_100=pm_pcs_cm3(data_pm0_5_c_div_100)/100
                    break;
                
                case CHANNEL_PM_CHANNEL_5:
                    let data_pm5_c_div_100 = (parseInt(stringHex.substring(i+2,i+6),16)) & 0xFFFF;
    ...
    function productStatusDataOutput(stringHex)
    {
        ...
        var data_productType = parseInt(stringHex.substring(0,2),16);
        var data_messageType = parseInt(stringHex.substring(2,4),16);
        var data_hw_version = (parseInt(stringHex.substring(4,6),16)) & 0xFF;
        var data_sw_version = (parseInt(stringHex.substring(6,8),16)) & 0xFF;
        var data_battery_voltage = (parseInt(stringHex.substring(8,12),16)) & 0xFFFF;
        var data_power_in = (parseInt(stringHex.substring(12,14),16)) & 0xFF;
        var data_battery_balance = (((parseInt(stringHex.substring(14,16),16)) & 0xFF)-124);
        var data_product_orientation = (parseInt(stringHex.substring(16,18),16)>>6) & 0x03;
        var data_working_mode = (parseInt(stringHex.substring(16,18),16)>>4) & 0x03;
        var data_power_source = (parseInt(stringHex.substring(16,18),16)>>3) & 0x01;
        ...
        data = {
            ...
            "workingMode" : workingMode(data_working_mode),
