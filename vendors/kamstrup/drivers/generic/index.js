/*
    Code added by Actility is put between an opening "ADDED BY ACTILITY" comment and a closing "END" comment.
    Code modified by Actility is put between an opening "MODIFIED BY ACTILITY" comment and a closing "END" comment.
*/

// ADDED BY ACTILITY
    const crypto = require('crypto');

    function xorBuffers(leftBuffer, rightBuffer) {
        const resultBuffer = Buffer.alloc(leftBuffer.length);
    ...
    function leftShiftOneBit(buffer) {
        const resultBuffer = Buffer.alloc(buffer.length);
        let carry = 0;
        for (let index = buffer.length - 1; index >= 0; index--) {
            const value = buffer[index];
            resultBuffer[index] = ((value << 1) & 0xff) | carry;
    ...
    function generateCmacSubkeys(key) {
        const constRb = Buffer.from('00000000000000000000000000000087', 'hex');
        const zeroBlock = Buffer.alloc(16, 0);
        const encryptedZero = aesEncryptBlock(key, zeroBlock);

        let firstSubkey = leftShiftOneBit(encryptedZero);
        if (encryptedZero[0] & 0x80) {
            firstSubkey = xorBuffers(firstSubkey, constRb);
        }

        let secondSubkey = leftShiftOneBit(firstSubkey);
        if (firstSubkey[0] & 0x80) {
            secondSubkey = xorBuffers(secondSubkey, constRb);
        }

        return { firstSubkey, secondSubkey };
    }

    function padCmacBlock(block) {
        const paddedBlock = Buffer.alloc(16, 0);
        block.copy(paddedBlock);
        paddedBlock[block.length] = 0x80;
        return paddedBlock;
    }

    function aesCmac(key, message) {
        const { firstSubkey, secondSubkey } = generateCmacSubkeys(key);
        ...
        const lastMessageBlock = message.subarray(lastBlockStart);
        const lastBlock = lastBlockComplete
            ? xorBuffers(lastMessageBlock, firstSubkey)
            : xorBuffers(padCmacBlock(lastMessageBlock), secondSubkey);

        let chainBlock = Buffer.alloc(16, 0);
        for (let blockIndex = 0; blockIndex < blockCount - 1; blockIndex++) {
            const messageBlock = message.subarray(blockIndex * 16, (blockIndex + 1) * 16);
            chainBlock = aesEncryptBlock(key, xorBuffers(chainBlock, messageBlock));
        }

        return aesEncryptBlock(key, xorBuffers(chainBlock, lastBlock)).toString('hex');
    }
    ...
    function securityLayer(i, configfield, raw, input, result) {
...
function decodeUplink(input) {
    ...
    // MODIFIED BY ACTILITY
        let decryptedData = null;
        if ((configfield & 0x1F00) != 0x0) { // Security mode different from 0
            const securityResults = securityLayer(i, configfield, raw, input, result);

            i = securityResults.i;
            raw = securityResults.raw;
            decryptedData = securityResults.decryptedData;
            result = securityResults.result ?? result;
        ...
        if (currentRecord.vib.type == "Date/time") { // Skip timestamps as they are mapped directly onto records
            ...
            if (currentRecord.data !== undefined) {
                ...
                if (currentRecord.vib.type.includes("timepoint")) {
                    value = currentRecord.data.toISOString();
                } else {
